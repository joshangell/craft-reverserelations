 ...
 */
class ReverseEvents extends EventsField
{
    ...
    {
        return Craft::t('reverserelations', 'Reverse Events Relations');
    }
    ...
    public function normalizeValue($value, ElementInterface $element = null)
    {
        ...
        if (!is_array($value) && $value !== '' && $element && $element->id) {
            ...
            if ($inputSourceIds != '*') {
                $query->where(['events_events.typeId' => $inputSourceIds]);
            }
        }
    ...
    public function beforeElementSave(ElementInterface $element, bool $isNew): bool
    {
        if (!$isNew) {
            // Get cached element
            $event = Events::$plugin->getEvents()->getEventById($element->id, $element->siteId);

            // Get old sources
            if ($event && $event->{$this->handle}) {
                $this->oldSources = $event->{$this->handle}->all();
            }
        }
    ...
    public function getEagerLoadingMap(array $sourceElements)
    {
        ...
        $map = (new Query())
            ...
            ->innerJoin('{{%events_events}} events', '[[relations.sourceId]] = [[events.id]]')
            ->where([
                'and',
                [
            ...
            ->where(['events.typeId' => $this->inputSourceIds()])
    ...
    protected function getFields(): array
    {
        ...
        foreach (Craft::$app->fields->getAllFields(false) as $field) {
            if ($field instanceof BaseRelationField && !($field instanceof $this)) {
    ...
    protected function inputSourceIds()
    {
        ...
        return Db::idsByUids('{{%events_eventtypes}}', $sources);
    }
}
